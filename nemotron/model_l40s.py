"""Cloudera AI Workbench Model para Nemotron 3 Nano FP8 en 1 x L40S 48 GB."""
...
# Recomendación oficial de NVIDIA para el checkpoint FP8.
os.environ.setdefault("VLLM_USE_FLASHINFER_MOE_FP8", "1")
os.environ.setdefault("VLLM_FLASHINFER_MOE_BACKEND", "throughput")
...
MODEL_ID = os.getenv(
    "LLM_MODEL_ID", "nvidia/NVIDIA-Nemotron-3-Nano-30B-A3B-FP8"
)
MAX_MODEL_LEN = int(os.getenv("LLM_MAX_MODEL_LEN", "262144"))
MAX_OUTPUT_TOKENS = int(os.getenv("LLM_MAX_OUTPUT_TOKENS", "8192"))

# FP8 es obligatorio para mantener pesos, runtime y cachés dentro de 48 GB.
_engine = LLM(
    model=MODEL_ID,
    trust_remote_code=True,
    tensor_parallel_size=1,
    dtype="auto",
    kv_cache_dtype=os.getenv("LLM_KV_CACHE_DTYPE", "fp8"),
    gpu_memory_utilization=float(os.getenv("LLM_GPU_MEMORY_UTILIZATION", "0.90")),
    max_model_len=MAX_MODEL_LEN,
    max_num_seqs=int(os.getenv("LLM_MAX_NUM_SEQS", "1")),
    max_num_batched_tokens=int(os.getenv("LLM_MAX_NUM_BATCHED_TOKENS", "4096")),
    enable_chunked_prefill=True,
    mamba_ssm_cache_dtype=os.getenv("LLM_MAMBA_SSM_CACHE_DTYPE", "float16"),
